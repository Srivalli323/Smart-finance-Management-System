...
import { getUserHouseholdService } from "./budget.service";
import { getBudgetStatusByHouseholdService } from "./budget.service";
import { emitBudgetWarning } from "../config/socket.config";
import { checkBudgetThresholdsService } from "./alert.service";
...
export const createTransactionService = async (
  ...
  // Get user's household if householdId not provided
  ...
  // Check budgets and trigger alerts (only for expenses)
  if (body.type === TransactionTypeEnum.EXPENSE) {
    try {
      ...
      for (const budget of userBudgets) {
        await checkBudgetThresholdsService(String(budget._id));
      }

      // Check household budgets
      if (householdId) {
        ...
        for (const budget of householdBudgets) {
          await checkBudgetThresholdsService(String(budget._id));
        }

        // Legacy: Check household monthly budget limit
        try {
          const budgetStatus = await getBudgetStatusByHouseholdService(householdId);
          if (budgetStatus.overBudget) {
            emitBudgetWarning(householdId, budgetStatus);
          }
        } catch (error) {
          // Silently fail
        }
      ...
      // Silently fail budget check - don't block transaction creation
...
export const updateTransactionService = async (
  ...
  // Get householdId if not in body
  let householdId = body.householdId || existingTransaction.householdId?.toString();
  if (!householdId) {
    const household = await getUserHouseholdService(userId);
    householdId = household?._id?.toString();
  }
  ...
  // Check budgets and trigger alerts (only for expenses)
  const isExpense = body.type === TransactionTypeEnum.EXPENSE || existingTransaction.type === TransactionTypeEnum.EXPENSE;
  if (isExpense) {
    try {
      // Check user budgets
      const userBudgets = await BudgetModel.find({
